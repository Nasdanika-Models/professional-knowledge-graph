package org.nasdanika.models.professionalknowledgegraph.ecore;
...
@EPackageNodeProcessorFactory(nsURI = ProfessionalKnowledgeGraphPackage.eNS_URI)
public class EcoreGenProfessionalKnowledgeGraphProcessorsFactory {

	private Context context;
	
	public EcoreGenProfessionalKnowledgeGraphProcessorsFactory(Context context) {
		this.context = context;
	}
		
	@EPackageNodeProcessorFactory(
			label = "Professional Knowledge Graph Model",
			description = "Model of the Professional Knowledge Graph domain",
			actionPrototype = """
                    app-action:
                      content:
                        content-markdown:
                          source:
                            content-resource:
                              location: professional-knowledge-graph.md
					"""
	)
		...
		return new EPackageNodeProcessor(config, context, prototypeProvider) {
			...
			protected void generateDiagramAndGraphActions(Collection<Label> labels, ProgressMonitor progressMonitor) {
				...
				try {
					...
					page.setName("Professional Knowledge Graph");
					...
					Files.writeString(new File("target/professional-knowledge-graph.drawio").toPath(), document.save(null));
				} catch (Exception e) {
					e.printStackTrace();
